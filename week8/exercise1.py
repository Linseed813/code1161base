...
def put_behind_bars(input_string="very naughty boy"):
    ...
    def add_pipes(letter):
        return letter + "|"

    string_list = map(add_pipes, string_list)
...
def best_letter_for_pets():
    ...
    """
    import string
    the_alphabet = list(string.lowercase)

    def pet_filter_length(letter):
        return len(pet_filter(letter))

    count_list = map(pet_filter_length, the_alphabet)
    max_count = max(count_list)
    index_letter = count_list.index(max_count)
    letter = the_alphabet[index_letter]
    return letter
...
def make_filler_text_dictionary():
    ...
    def make_word(number):
        ...
        return word
    for i in range(3, 8):
        text_dictionary[i].append(make_word(i))
        text_dictionary[i].append(make_word(i))
        text_dictionary[i].append(make_word(i))
